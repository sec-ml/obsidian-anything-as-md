...
export default class AnythingAsMdPlugin extends Plugin {
  ...
  private patchGetMarkdownFiles() {
    ...
    vault.getMarkdownFiles = () => {
      ...
      const extras = this.app.vault
        .getFiles()
        .filter(
          (file) => file instanceof TFile && this.isExtraMarkdownFile(file),
        );
  ...
  private async refreshMetadataForFile(
    file: TFile,
    showNoticeOnFailure: boolean,
  ) {
    ...
    try {
      ...
      console.error(
        "Anything as Markdown: metadata refresh failed for",
        file.path,
        error,
      );
      if (showNoticeOnFailure) {
  ...
  private removeDefaultExtensionPill(
    ...
    container.querySelectorAll("span, div").forEach((el) => {
      if (
        ...
        el.classList.contains("anything-as-md-ext-label")
      )
        return;
